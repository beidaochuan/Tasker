...
- [Python](https://www.python.org/) 3.6 以上、および C++ ビルドツール（Windows は [Visual Studio Build Tools](https://visualstudio.microsoft.com/downloads/) の「C++ によるデスクトップ開発」）
  - 依存パッケージ `better-sqlite3` はプリビルド済みバイナリを提供していないため、`npm ci` 実行時にソースからビルドされます。上記が無いと `npm ci`（新規セットアップ・更新・自己更新のいずれも）が失敗します
  - Windows サービスとして自己更新する場合は、Taskerサービスの実行アカウント（既定は `LocalSystem`）から見えるように、**ユーザー専用ではなくマシン全体（All Users）向けにインストール**してください
...
セットアップスクリプトは、同じ配布 ZIP に含まれる Tasker 本体を使用します。指定したインストール先にTaskerがすでに存在する場合も、同梱されたバージョンとの比較と更新にローカルファイルを使用するため、GitHubから同じZIPを再ダウンロードしません。依存関係の手動インストール、環境変数の設定、`npm start` の実行は不要です。セットアップ完了後は、元の ZIP と一時的な解凍フォルダを削除できます。

実行元に完全な配布ファイルがない場合、または `-ReleaseTag` が同梱バージョンと異なる場合は、指定されたReleaseをGitHub Releasesからダウンロードします。
...
Python と C++ ビルドツールは自動インストールされません。[前提条件](#前提条件)に従って事前に用意してください。
...
既存のサービスを更新する場合も、セットアップスクリプトを使います。インストール先がすでに存在すると、既存のサービス設定を維持した更新処理に自動で切り替わります。配布 ZIP 内から実行した場合は同梱ファイルを更新元に使用します。GitHub Releasesから取得する場合は、ダウンロードしたReleaseのSHA-256を検証してから更新します。失敗時はアプリ本体と依存関係を自動復旧します。
...
「この端末を更新」は `LocalSystem` アカウントで実行されるため、[前提条件](#前提条件)のPython・C++ビルドツールは必ず**マシン全体（All Users）向け**にインストールしてください。ユーザー専用インストールのみの場合、`LocalSystem` から見えず更新が失敗します。
